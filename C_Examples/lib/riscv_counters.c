...
void *addr_to_pointer (uint64_t addr)
{
#ifdef RV32
    uint32_t addr1 = addr;    // Discards addr [63:32]
#endif
#ifdef RV64
    uint64_t addr1 = addr;
#endif
...
{
    uint32_t *p = (uint32_t *) addr_to_pointer (addr);
    return *p;
}
...
{
    uint64_t *p = (uint64_t *) addr_to_pointer (addr);
    return *p;
}
