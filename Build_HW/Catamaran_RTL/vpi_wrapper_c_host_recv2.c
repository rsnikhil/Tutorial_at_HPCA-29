 ...
 *     #include "vpi_wrapper_c_host_recv2.h"
 *     void (*vlog_startup_routines[])() = { c_host_recv2_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_c_host_recv2 call=c_host_recv2_calltf size=8 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_c_host_recv2 vpiSysFuncSized 8 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
char c_host_recv2(char );

/* VPI wrapper function */
PLI_INT32 c_host_recv2_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  char arg_1;
  char vpi_result;
...
/* sft: $imported_c_host_recv2 vpiSysFuncSized 8 unsigned */

/* tab: $imported_c_host_recv2 call=c_host_recv2_calltf size=8 acc=rw:%TASK */

PLI_INT32 c_host_recv2_sizetf(PLI_BYTE8 *user_data)
{
  return 8u;
}

/* VPI wrapper registration function */
void c_host_recv2_vpi_register()
{
  ...
  tf_data.tfname = "$imported_c_host_recv2";
  tf_data.calltf = c_host_recv2_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = c_host_recv2_sizetf;
  tf_data.user_data = "$imported_c_host_recv2";
