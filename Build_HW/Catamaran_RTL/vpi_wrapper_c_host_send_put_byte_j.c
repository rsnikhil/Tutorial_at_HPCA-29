 ...
 *     #include "vpi_wrapper_c_host_send_put_byte_j.h"
 *     void (*vlog_startup_routines[])() = { c_host_send_put_byte_j_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_c_host_send_put_byte_j call=c_host_send_put_byte_j_calltf size=8 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_c_host_send_put_byte_j vpiSysFuncSized 8 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
char c_host_send_put_byte_j(unsigned int , char );

/* VPI wrapper function */
PLI_INT32 c_host_send_put_byte_j_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  unsigned int arg_1;
  char arg_2;
  ...
  vpi_result = c_host_send_put_byte_j(arg_1, arg_2);
...
/* sft: $imported_c_host_send_put_byte_j vpiSysFuncSized 8 unsigned */

/* tab: $imported_c_host_send_put_byte_j call=c_host_send_put_byte_j_calltf size=8 acc=rw:%TASK */

PLI_INT32 c_host_send_put_byte_j_sizetf(PLI_BYTE8 *user_data)
{
  return 8u;
}

/* VPI wrapper registration function */
void c_host_send_put_byte_j_vpi_register()
{
  ...
  tf_data.tfname = "$imported_c_host_send_put_byte_j";
  tf_data.calltf = c_host_send_put_byte_j_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = c_host_send_put_byte_j_sizetf;
  tf_data.user_data = "$imported_c_host_send_put_byte_j";
