 ...
 *     #include "vpi_wrapper_c_host_try_accept.h"
 *     void (*vlog_startup_routines[])() = { c_host_try_accept_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_c_host_try_accept call=c_host_try_accept_calltf size=8 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_c_host_try_accept vpiSysFuncSized 8 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
char c_host_try_accept(char );

/* VPI wrapper function */
PLI_INT32 c_host_try_accept_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  char arg_1;
  char vpi_result;
  ...
  vpi_result = c_host_try_accept(arg_1);
...
/* sft: $imported_c_host_try_accept vpiSysFuncSized 8 unsigned */

/* tab: $imported_c_host_try_accept call=c_host_try_accept_calltf size=8 acc=rw:%TASK */

PLI_INT32 c_host_try_accept_sizetf(PLI_BYTE8 *user_data)
{
  return 8u;
}

/* VPI wrapper registration function */
void c_host_try_accept_vpi_register()
{
  ...
  tf_data.tfname = "$imported_c_host_try_accept";
  tf_data.calltf = c_host_try_accept_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = c_host_try_accept_sizetf;
  tf_data.user_data = "$imported_c_host_try_accept";
